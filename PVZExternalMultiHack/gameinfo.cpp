...
void GameInfo::populateHacks() const
{
    ...
    std::get<uintptr_t>(hacks.infiniteCoins )= SigScan(m_hPVZ, m_mPVZ32, {0x8B, 0x51, 0x54, 0x52, 0x8D, 0x44, 0x24, 0x30});
    ...
    // ====================== multi address hacks ===========================
    std::get<std::vector<uintptr_t>>(hacks.infinitePlantHealth).push_back(SigScan(m_hPVZ, m_mPVZ32, {0x29, 0x50, 0x40, 0x83, 0xF9, 0x19}));
    std::get<std::vector<uintptr_t>>(hacks.infinitePlantHealth).push_back(SigScan(m_hPVZ, m_mPVZ32, {0x83, 0x46, 0x40, 0xFC, 0x8B, 0x4E, 0x40}));

    std::get<std::vector<uintptr_t>>(hacks.oneHitKills).push_back(SigScan(m_hPVZ, m_mPVZ32, {0x8B, 0xAF, 0xC8, 0x00, 0x00, 0x00, 0x8B}));
    std::get<std::vector<uintptr_t>>(hacks.oneHitKills).push_back(SigScan(m_hPVZ, m_mPVZ32, {0x8B, 0x8D, 0xD0, 0x00, 0x00, 0x00, 0xB8}));
    std::get<std::vector<uintptr_t>>(hacks.oneHitKills).push_back(SigScan(m_hPVZ, m_mPVZ32, {0x29, 0x86, 0xDC, 0x00, 0x00, 0x00}));
}
